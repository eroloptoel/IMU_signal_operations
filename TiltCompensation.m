...
%     Yaw (Clock - wise rotation)  = +ve reading
  ...
  Gyro_pitch = Gyro_roll .* sin(Gyro_z);         % Sensor_to_rad  deg2rad(0.000128)   % Transfer the roll angle to the pitch angle if the Z-axis has yawed
  Gyro_roll = Gyro_pitch .* sin(Gyro_z);                                              % Transfer the pitch angle to the roll angle if the Z-axis has yawed
...
%  
  ...
  Gyro_pitch_output =0;Gyro_roll_output =0;
...
c=0;d=0;
 for i=1:7485
     if (Heading(i,1)<0);    Heading(i,1) = -Heading(i,1);c=c+1;end %&& phi1(i)>0 && theta1(i) >0
