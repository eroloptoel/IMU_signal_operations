classdef ECF_AHRS < handle
 ...
 quater = obj.Quaternion; % short name local variable forreadability
 ...
 h = quaternProd(quater, quaternProd([0 Magnetometer],quaternConj(quater)));
 b = [0 norm([h(2) h(3)]) 0 h(4)];

 % Estimated direction of gravity and magnetic field
 v = [2*(quater(2)*quater(4)-quater(1)*quater(3))  2*(quater(1)*quater(2)+quater(3)*quater(4))  quater(1)^2-quater(2)^2-quater(3)^2+quater(4)^2];
 w = [2*b(2)*(0.5-quater(3)^2-quater(4)^2)+2*b(4)*(quater(2)*quater(4)-quater(1)*quater(3))  2*b(2)*(quater(2)*quater(3)-quater(1)*quater(4))+2*b(4)*(quater(1)*quater(2)+quater(3)*quater(4))  2*b(2)*(quater(1)*quater(3)+quater(2)*quater(4)) + 2*b(4)*(0.5-quater(2)^2-quater(3)^2)];
...
% end
 ...
 qDot = 0.5 * quaternProd(quater, [0 Gyroscope(1) Gyroscope(2) Gyroscope(3)]);
 % Integrate to yield quaternion
 quater = quater + qDot * obj.SamplePeriod;
 obj.Quaternion = quater / norm(quater); % normalise quaternion
 ...
 v = [2*(obj.q(2)*obj.q(4)-obj.q(1)*obj.q(3))  2*(obj.q(1)*obj.q(2)+obj.q(3)*obj.q(4))  obj.q(1)^2-obj.q(2)^2-obj.q(3)^2+obj.q(4)^2]; % estimated direction of gravity
